<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/green/space_event_list|template/green/header|template/green/space_menu|template/green/footer', '1369994733', 'template/green/space_event_list');?><?php $_TPL['titles'] = array('活动'); ?>
...
<head>
<style type="text/css">
@import url(template/default/paiban.css);
</style>
</head>
...
<h2 class="title"><img src="image/icon/event.gif" />活动</h2>
...
<div id="content">
   <div class="col01">
    <div class="post">
      <h3><a href="space.php?do=event&id=<?=$event['eventid']?>"><?=$event['title']?><span class="gray">[<?=$_SGLOBAL['eventclass'][$event['classid']]['classname']?>]</span></a></h3>
      <div class="post-inner">
       
        <a href="space.php?do=event&id=<?=$event['eventid']?>"><img src="<?=$event['pic']?>" alt="<?=$event['title']?>" class="thumbnail" onerror="this.src='<?=$_SGLOBAL['eventclass'][$event['classid']]['poster']?>'"/></a>
        <br/>
       						<p> 
...
<?php } ?>
<?php if($event['endtime']<$_SGLOBAL['timestamp']) { ?>
<span class="event_state" style="position: absolute;
right: 0;
margin-right:220px;
width: 60px;
height: 60px;
text-indent: -999px;
background: url(template/default/image/print.gif) no-repeat top right;
...
<p><span class="gray">发起人:</span> <a href="space.php?uid=<?=$event['uid']?>"><?=$_SN[$event['uid']]?></a></p><br/>
<?php if($fevents[$event['eventid']]) { ?>
<p><span class="gray">好友:</span> 
...
<?php } ?>
      ...
      <br/><br/>
      <div class="meta"><?=$event['viewnum']?> 次查看&nbsp; <?=$event['membernum']?> 人参加&nbsp; <?=$event['follownum']?> 人关注</div>
      <?php } ?>
    </div>
