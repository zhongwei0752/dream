<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/green/cp_news|template/green/header|template/green/cp_topic_menu|template/green/footer|template/green/space_topic_inc', '1369490111', 'template/green/cp_news');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<form method="post" action="cp.php?ac=news&op=delete&newsid=<?=$newsid?>">
<p>确定删除指定的今日资讯吗？</p>
<p class="btn_line">
...
<a href="javascript:hideMenu();" class="float_del" title="关闭">关闭</a>
<div class="popupmenu_inner">
<form method="post" action="cp.php?ac=news&op=edithot&newsid=<?=$newsid?>">
<p class="btn_line">
...
<ul class="tabs">
<?php if($news['newsid']) { ?>
<li class="active"><a href="cp.php?ac=news&newsid=<?=$news['newsid']?>"><span>编辑今日资讯</span></a></li>
<?php } ?>
<li<?php if(empty($news['newsid'])) { ?> class="active"<?php } ?>><a href="cp.php?ac=news"><span>发表新今日资讯</span></a></li>
<li><a href="space.php?uid=<?=$space['uid']?>&do=news&orderby=dateline"><span>返回我的今日资讯</span></a></li>
</ul>
</div>
<?php } ?>
...
<form method="post" action="cp.php?ac=news&newsid=<?=$news['newsid']?>" enctype="multipart/form-data">
...
<input type="text" class="t_input" id="subject" name="subject" value="<?=$news['subject']?>" size="60" onblur="relatekw();" /><br/>
封面图片:<input type="file" name="files" value="<?=$news['imageurl']?>" />
</td>
</tr>
<tr>
<td>
...
<td>
<input type="checkbox" name="makefeed" id="makefeed" value="1"<?php if(ckprivacy('news', 1)) { ?> checked<?php } ?>> 产生动态 (<a href="cp.php?ac=privacy#feed" target="_blank">更改默认设置</a>)
</td>
</tr>			
</table>
