<?php		//用PHP批量生成图片缩略图
	...
	class image
	{
		...
		function Mini1($src,$w,$h,$q=80,$id,$table)		//生成缩略图 Mini(图片地址, 宽度, 高度, 质量)
		{
			...
			$this->w=$w;
			$this->h=$h;
			$this->newdir=$newdir ? $newdir : "./upload/$table/littlesmall";
			if(strrchr($src,".")==".gif" && $this->regif==0)	//是否处理GIF图
			{
				return $this->src;
			}
		...
		function Mini2($src,$w,$h,$q=80,$id,$table)		//生成缩略图 Mini(图片地址, 宽度, 高度, 质量)
		{
			...
			$this->w=$w;
			$this->h=$h;
			$this->newdir=$newdir ? $newdir : "./upload/$table/navnewssmall";
			if(strrchr($src,".")==".gif" && $this->regif==0)	//是否处理GIF图
			{
				return $this->src;
			}
		...
		function Mini3($src,$w,$h,$q=80,$id,$table)		//生成缩略图 Mini(图片地址, 宽度, 高度, 质量)
		{
			...
			$this->w=$w;
			$this->h=$h;
			$this->newdir=$newdir ? $newdir : "./upload/$table/zhuantinewssmall";
			if(strrchr($src,".")==".gif" && $this->regif==0)	//是否处理GIF图
			{
				return $this->src;
			}
