...
/*
	...
	$Id: function_littlenews.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
//Ìí¼Ó²©¿Í
function littlenews_post($POST, $olds=array()) {
	...
	if(empty($olds['classid']) || $POST['classid'] != $olds['classid']) {
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				$classid = getcount('classlittlenews', array('classname'=>$classname, 'uid'=>$_SGLOBAL['supe_uid']), 'classid');
				if(empty($classid)) {
					$setarr = array(
						'classname' => $classname,
					...
					$classid = inserttable('classlittlenews', $setarr, 1);
				}
			}
		} else {
	...
	if($classid && empty($classname)) {
		//ÊÇ·ñÊÇ×Ô¼ºµÄ
		$classname = getcount('classlittlenews', array('classid'=>$classid, 'uid'=>$_SGLOBAL['supe_uid']), 'classname');
	...
	if(!empty($POST['picids'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if(empty($titlepic) && $value['thumb']) {
				...
				$littlenewsarr['picflag'] = $value['remote']?2:1;
			}
	...
	if(empty($titlepic)) {
		...
		$littlenewsarr['picflag'] = 0;
	}
	$littlenewsarr['pic'] = $titlepic;
	
	//ÈÈ¶È
	if(checkperm('managelittlenews')) {
		$littlenewsarr['hot'] = intval($POST['hot']);
	}
	
	if($olds['littlenewsid']) {
		//¸üÐÂ
		$littlenewsid = $olds['littlenewsid'];
		updatetable('littlenews', $littlenewsarr, array('littlenewsid'=>$littlenewsid));
		
		$fuids = array();
		
		$littlenewsarr['uid'] = $olds['uid'];
		$littlenewsarr['username'] = $olds['username'];
	} else {
		//²ÎÓëÈÈÄÖ
		$littlenewsarr['topicid'] = topic_check($POST['topicid'], 'littlenews');

		$littlenewsarr['uid'] = $_SGLOBAL['supe_uid'];
		$littlenewsarr['username'] = $_SGLOBAL['supe_username'];
		$littlenewsarr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$littlenewsid = inserttable('littlenews', $littlenewsarr, 1);
	}
	
	$littlenewsarr['littlenewsid'] = $littlenewsid;
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//ÏÈ°ÑÒÔÇ°µÄ¸øÇåÀíµô
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, littlenewsid FROM ".tname('taglittlenews')." WHERE littlenewsid='$littlenewsid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET littlenewsnum=littlenewsnum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('taglittlenews')." WHERE littlenewsid='$littlenewsid'");
			}
		}
		$tagarr = tag_batch($littlenewsid, $POST['tag']);
		//¸üÐÂ¸½±íÖÐµÄtag
	...
	if($olds) {
		//¸üÐÂ
		updatetable('littlenewsfield', $fieldarr, array('littlenewsid'=>$littlenewsid));
	} else {
		$fieldarr['littlenewsid'] = $littlenewsid;
		$fieldarr['uid'] = $littlenewsarr['uid'];
		inserttable('littlenewsfield', $fieldarr);
	}
	...
	if($isself) {
		if($olds) {
			...
			if(empty($space['littlenewsnum'])) {
				$space['littlenewsnum'] = getcount('littlenews', array('uid'=>$space['uid']));
				$littlenewsnumsql = "littlenewsnum=".$space['littlenewsnum'];
			} else {
				$littlenewsnumsql = 'littlenewsnum=littlenewsnum+1';
			}
			//»ý·Ö
			$reward = getreward('publishlittlenews', 0);
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$littlenewsnumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
			
			//Í³¼Æ
			updatestat('littlenews');
		}
	}
	//封面图片上传
	...
	if($POST['makefeed']) {
		...
		feed_publish($littlenewsid, 'littlenewsid', $olds?0:1);
	}
	
	//ÈÈÄÖ
	if(empty($olds) && $littlenewsarr['topicid']) {
		topic_join($littlenewsarr['topicid'], $_SGLOBAL['supe_uid'], $_SGLOBAL['supe_username']);
	}

	//½ÇÉ«ÇÐ»»
	if(!empty($__SGLOBAL)) $_SGLOBAL = $__SGLOBAL;

	return $littlenewsarr;
}

//´¦Àítag
function tag_batch($littlenewsid, $tags) {
	global $_SGLOBAL;
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET littlenewsnum=littlenewsnum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$littlenewsid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('taglittlenews')." (tagid,littlenewsid) VALUES ".implode(',', $inserts));
...
//ÊÓÆµ±êÇ©´¦Àí
function littlenews_bbcode($message) {
	$message = preg_replace("/\[flash\=?(media|real)*\](.+?)\[\/flash\]/ie", "littlenews_flash('\\2', '\\1')", $message);
	return $message;
}
//ÊÓÆµ
function littlenews_flash($swf_url, $type='') {
	$width = '520';
	$height = '390';
	if ($type == 'media') {
