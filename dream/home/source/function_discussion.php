...
/*
	...
	$Id: function_discussion.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
//Ìí¼Ó²©¿Í
function discussion_post($POST, $olds=array()) {
	...
	//案例治疗方法处理
	if($_SGLOBAL['mobile']) {
		$POST['think'] = getstr($POST['think'], 0, 1, 0, 1, 1);
	} else {
		$POST['think'] = checkhtml($POST['think']);
		$POST['think'] = getstr($POST['think'], 0, 1, 0, 1, 0, 1);
		$POST['think'] = preg_replace(array(
				"/\<div\>\<\/div\>/i",
				"/\<a\s+href\=\"([^\>]+?)\"\>/i"
			), array(
				'',
	...
	$think = $POST['think'];
	if(strlen($think)<1) showmessage('that_should_at_least_write_things');
	...
	if(!empty($POST['picids'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if(empty($titlepic) && $value['thumb']) {
				...
				$discussionarr['picflag'] = $value['remote']?2:1;
			}
	...
	if(empty($titlepic)) {
		...
		$discussionarr['picflag'] = 0;
	}
	$discussionarr['pic'] = $titlepic;
	
	//ÈÈ¶È
	if(checkperm('managediscussion')) {
		$discussionarr['hot'] = intval($POST['hot']);
	}
	
	if($olds['discussionid']) {
		//¸üÐÂ
		$discussionid = $olds['discussionid'];
		updatetable('discussion', $discussionarr, array('discussionid'=>$discussionid));
		
		$fuids = array();
		
		$discussionarr['uid'] = $olds['uid'];
		$discussionarr['username'] = $olds['username'];
	} else {
		//²ÎÓëÈÈÄÖ
		$discussionarr['topicid'] = topic_check($POST['topicid'], 'discussion');

		$discussionarr['uid'] = $_SGLOBAL['supe_uid'];
		$discussionarr['username'] = $_SGLOBAL['supe_username'];
		$discussionarr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$discussionid = inserttable('discussion', $discussionarr, 1);
	}
	
	$discussionarr['discussionid'] = $discussionid;
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//ÏÈ°ÑÒÔÇ°µÄ¸øÇåÀíµô
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, discussionid FROM ".tname('tagdiscussion')." WHERE discussionid='$discussionid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET discussionnum=discussionnum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('tagdiscussion')." WHERE discussionid='$discussionid'");
			}
		}
		$tagarr = tag_batch($discussionid, $POST['tag']);
		//¸üÐÂ¸½±íÖÐµÄtag
	...
	if($olds) {
		//¸üÐÂ
		updatetable('discussionfield', $fieldarr, array('discussionid'=>$discussionid));
	} else {
		$fieldarr['discussionid'] = $discussionid;
		$fieldarr['uid'] = $discussionarr['uid'];
		inserttable('discussionfield', $fieldarr);
	}
	...
	if($isself) {
		if($olds) {
			...
			if(empty($space['discussionnum'])) {
				$space['discussionnum'] = getcount('discussion', array('uid'=>$space['uid']));
				$discussionnumsql = "discussionnum=".$space['discussionnum'];
			} else {
				$discussionnumsql = 'discussionnum=discussionnum+1';
			}
			//»ý·Ö
			$reward = getreward('publishdiscussion', 0);
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$discussionnumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
			
			//Í³¼Æ
			updatestat('discussion');
		}
	}
	...
	if($POST['makefeed']) {
		...
		feed_publish($discussionid, 'discussionid', $olds?0:1);
	}
	
	//ÈÈÄÖ
	if(empty($olds) && $discussionarr['topicid']) {
		topic_join($discussionarr['topicid'], $_SGLOBAL['supe_uid'], $_SGLOBAL['supe_username']);
	}
...
//´¦Àítag
function tag_batch($discussionid, $tags) {
	global $_SGLOBAL;
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET discussionnum=discussionnum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$discussionid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('tagdiscussion')." (tagid,discussionid) VALUES ".implode(',', $inserts));
...
//ÊÓÆµ±êÇ©´¦Àí
function discussion_bbcode($message) {
	$message = preg_replace("/\[flash\=?(media|real)*\](.+?)\[\/flash\]/ie", "discussion_flash('\\2', '\\1')", $message);
	return $message;
}
//ÊÓÆµ
function discussion_flash($swf_url, $type='') {
	$width = '520';
	$height = '390';
	if ($type == 'media') {
