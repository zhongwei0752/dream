...
/*
	...
	$Id: function_news.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
function news_post($POST, $olds=array()) {
	...
	if(empty($olds['classid']) || $POST['classid'] != $olds['classid']) {
		if(!empty($POST['classid']) && substr($POST['classid'], 0, 4) == 'new:') {
			...
			if(empty($classname)) {
				...
				$classid = getcount('classnews', array('classname'=>$classname, 'uid'=>$_SGLOBAL['supe_uid']), 'classid');
				if(empty($classid)) {
					$setarr = array(
						'classname' => $classname,
					...
					$classid = inserttable('classnews', $setarr, 1);
				}
			}
		} else {
	...
	if(!empty($POST['picids'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if(empty($titlepic) && $value['thumb']) {
				...
				$newsarr['picflag'] = $value['remote']?2:1;
			}
	...
	if($olds['newsid']) {
		...
		$newsarr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$newsid = inserttable('news', $newsarr, 1);
	}
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			...
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET newsnum=newsnum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('tagnews')." WHERE newsid='$newsid'");
			}
		}
	...
	if($isself) {
		if($olds) {
			...
			if(empty($space['newsnum'])) {
				$space['newsnum'] = getcount('news', array('uid'=>$space['uid']));
				$newsnumsql = "newsnum=".$space['newsnum'];
			} else {
...
function tag_batch($newsid, $tags) {
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET newsnum=newsnum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$newsid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('tagnews')." (tagid,newsid) VALUES ".implode(',', $inserts));
